...
const CardExtra: FC<{ message: ExMessageItem }> = ({ message }) => (
  <OIMAvatar src={message.cardElem.faceURL} size={26} text={message.cardElem.nickname} />
);
...
const QuoteMessageRenderer: FC<IMessageItemProps> = ({ message: { quoteElem } }) => {
  ...
  const isPictureMessage = quoteElem.quoteMessage.contentType === MessageType.PictureMessage;
  const isVideoMessage = quoteElem.quoteMessage.contentType === MessageType.VideoMessage;
  ...
  const isLocationMessage = quoteElem.quoteMessage.contentType === MessageType.LocationMessage;
  const isRevokedMessage = quoteElem.quoteMessage.contentType === MessageType.RevokeMessage;
  ...
  return (
    <div
      className={clsx('mt-1 flex w-fit items-center rounded-md bg-[var(--chat-bubble)] p-2.5', {
        'py-1.5': hasExtra,
      })}
    >
      <Twemoji>
